...
public class AddDatabaseServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		boolean edit = from.getBool("edit", false);
		...
		if (configuration != null) {
			if (!configuration.getBool("disabled", false) && !edit) {
				json.put("status", ConfigUtils.RESP_STATUS_FAIL);
				json.put("error", String.format("A database proxy for %s already exists", name));
				resp.getWriter().append(json.toJSONString());
				return;
			}
			if (configuration.get("disabled") != null) {
				configuration.remove("disabled");
			}
			configuration.merge(from, true);
		} else {
			databaseProxys.put(name, from);
		}
		ConfigUtils.writeDatabaseProxys(databaseProxys);
		json.put("status", ConfigUtils.RESP_STATUS_OK);
		json.put("msg", String.format("Configuration %s for \"%s\" success!", edit ? "edit" : "added", name));
