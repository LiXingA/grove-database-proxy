...
public class MongoUtil {
    ...
    public static MongoClient initMongoClient(Configuration conf) {
        ...
        Integer port = conf.getInt(KeyConstant.MONGO_PORT);
        if (!Strings.isNullOrEmpty(host) && port != null) {
            addressList = Arrays.asList(String.format("%s:%d", host, port));
        } else {
            addressList = conf.getList(KeyConstant.MONGO_ADDRESS);
        }
